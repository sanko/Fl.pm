...
C<FL_ABI_VERSION> is an int that describes the major, minor, and patch ABI
version numbers in the same format as C<FL_API_VERSION>.

The ABI version number C<FL_ABI_VERSION> is usually the same as the API version
<FL_API_VERSION> with the last two digits set to '00'.

FLTK retains the ABI (Application Binary Interface) during patch releases of
the same major and minor versions. Examples:
...
Version C<1.2.3> is actually stored as C<10203> to allow for more than 9 minor
and patch releases.

The C<FL_MAJOR_VERSION>, C<FL_MINOR_VERSION>, and C<FL_PATCH_VERSION> constants
give the integral values for the major, minor, and patch releases respectively.

To enable new ABI-breaking features in patch releases you can configure FLTK to
use a higher C<FL_ABI_VERSION>.
...
export_constant("FL_ABI_VERSION",   "version");
export_constant("FL_API_VERSION",   "version");
