...
The Fl::HtmlView widget displays HTML text. It's based on Fl_Help_View. Why is
the widget not Fl_Html_View upstream? I don't know either... Anwyay! Here we
go!
...
Fl::HtmlView inherits from Fl::Group and Fl::Widget. On top of that, it exposes
...
If the given pixel value left is out of range, then the text is scrolled to the
left or right side of the document.
...
The callback function receives a pointer to the Fl::HtmlView widget and the URI
or full pathname for the file in question. It must return a pathname that can
be opened as a local file or C<undef>.
