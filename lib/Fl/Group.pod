...
group, and to other groups. The only modifier grabbed is shift (for shift-tab),
so that ctrl-tab, alt-up, and such are free for the app to use as shortcuts.
...
Sets the current group so youcan build the widget tree by just constructing the
widgets.
...
When the group is resized, it calculates a new size and position for all of its
children. Widgets that rae horizontally or vertically inside the dimensions of
the box are scaled to the new size. Widgets outside othe box are moved.
