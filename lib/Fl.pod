...
test add_awake_handler_ => sub {
    add_awake_handler_(sub { diag('TEST') }, {test => 'data'});
...
If FLTK cannot figure out what to do with an event, it calls each of these
functions (most recent first) until one of them returns non-zero. If none of
them returns non-zero then the event is ignored. Events that cause this to be
called are:

=over

=item * C<FL_SHORTCUT> events that are not recognized by any widget. This lets you provide global shortcut keys.

=item * C<FL_SCREEN_CONFIGURATION_CHANGED> events. Under X11, this event requires the libXrandr.so shared library to be loadable at run-time and the X server to implement the RandR extension.

=item * C<FL_ZOOM_EVENT> events.

=item * System events that FLTK does not recognize. See fl_xevent.

=item * Some other events when the widget FLTK selected returns zero from its C<handle()> method. Exactly which ones may change in future versions, however.
...
=item * C<Fl::remove_handler( ... )>

=item * C<Fl::event_dispatch( ... )>
...
xs {name        => 'Fl::add_handler',
    definitions => [{required    => [['SV *', 'coderef']],
                     inject_code => '_cb_ev_CODE = new Callback( coderef );',
                     c_args      => '_cb_ev'
                    }
    ],
    export => ['all']
};
test add_handler => sub {
    add_handler(sub {...});
    pass('add_handler( sub {...} ) did not kill us');
};
...
options, you will need to pass your own argument handler to C<Fl::args(...)>
explicitly.
...
=head2 C<background( ... )>
	...
	background( 124, 124, 98 ); # R, G, B

Changes C<fl_color(FL_BACKGROUND_COLOR)> to the given color, and changes the
gray ramp from 32 to 56 to black to white.

These are the colors used as backgrounds by almost all widgets and used to draw
the edges of all the boxtypes.

=cut

xs {name => 'Fl::background',
    definitions =>
        [{required => [['uchar', 'r'], ['uchar', 'g'], ['uchar', 'b']]}],
    export => ['all']
};
test background => sub {

# Can't test that this works without actually popping up a window and asking someone
    background(0, 0, 0);
    pass('background(...) did not crash us');
};

=head2 C<background2( ... )>

	background2( 124, 124, 98 ); # R, G, B

Changes the alternative background color.

This color is used as a background by C<Fl_Input> and other text widgets.

This call may change C<fl_color(FL_FOREGROUND_COLOR)> if it does not provide
sufficient contrast to C<FL_BACKGROUND2_COLOR>.

=cut

xs {name => 'Fl::background2',
    definitions =>
        [{required => [['uchar', 'r'], ['uchar', 'g'], ['uchar', 'b']]}],
    export => ['all']
};
test background2 => sub {

# Can't test that this works without actually popping up a window and asking someone
    background2(0, 0, 0);
    pass('background2(...) did not crash us');
};
