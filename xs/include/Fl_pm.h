...
public:
	...
	int trigger(int argc, char **argv, int &i) {
		int RETVAL = 0;
		int count;
		dTHX;
		//warn("trigger( argc, argv, &i )");
		dSP;
		ENTER;
		SAVETMPS;
		PUSHMARK(SP);
		...
		{
			AV *_argv_AV;
			_argv_AV = (AV *)sv_2mortal((SV *)newAV());
			int size = ((int)(sizeof(argv) / sizeof(argv)[0]));
			for (int i = 0; i <= size; i++)
				av_push(_argv_AV, (SV *) newSVpv(argv[i], strlen(argv[i])));
			_argv = newRV((SV *)_argv_AV);
			_argv = sv_2mortal(_argv);
		}
		XPUSHs((SV *) _argv);
